...
export default function ForgotPasswordPage() {
    ...
    const handleSubmit = async (e: FormEvent) => {
        ...
        try {
            // 1) نتحقق من وجود الايميل عبر endpoint السيرفر
            ...
            if (!checkRes.ok) {
                ...
                throw new Error(json?.error || 'حدث خطأ أثناء التحقق من الإيميل.');
            }

            const { exists } = await checkRes.json();

            if (!exists) {
                // لو مش موجود - مانبعتش reset ونعرض رسالة
                setError("لا يوجد حساب مسجل بهذا البريد الإلكتروني");
                setLoading(false);
                return;
            }

            // 2) لو موجود — نطلب من Supabase إرسال رابط استعادة كلمة المرور
            const { error: sbError } = await supabase.auth.resetPasswordForEmail(email, {
                // اختياري: redirectTo: `${window.location.origin}/auth/callback`
            });
